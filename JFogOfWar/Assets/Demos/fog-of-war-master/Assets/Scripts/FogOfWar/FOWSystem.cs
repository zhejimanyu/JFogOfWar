...
public class FOWSystem : MonoSingleton<FOWSystem>
{
	...
	protected Color32[] mBuffer1;//r控制是否可见
    ...
    /// 您的世界的大小（单位）。 例如，如果您的地形为256x256，则将其保留为“ 256”。
    /// </summary>
    ...
    /// 可见性检查执行的频率
    /// 每隔updateFrequency秒进入一次NeedUpdate状态
    /// </summary>
    ...
    /// 纹理从一个融合到另一个需要多长时间。
    /// 可控制切换未知区域到已知区域的时间和效果，值小会有刷新卡顿的感觉,值大则混存切换的比例会比较小[相当于切换有延迟]，2是一个比较好的经验值
    /// </summary>
	...
	void Update ()
	{
        ...
        if (mState == State.Blending)//Blending
		{
    ...
    /// 多重采样，模糊边缘值
    /// 多重采样完成后先存在buffer1的r里，之后将buffer1的值赋值到buffer2上
    /// </summary>
    ...
    void MergeBuffer()
    {
        for (int index = 0; index < mTextureSizeSqr; ++index)
        {
            mBuffer0[index].b = mBuffer1[index].r;//这一句注释掉会使边缘变成一闪一闪的
